...
def make_env(
	...
	assert mode in {'train', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 'distracting_cs'} or 'video' in mode, \
	...
	if mode == 'video_easy' or mode =='distracting_cs':
		env = VideoWrapper(env, mode, seed)
	else:
...
class ColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		if 'color' in self._mode:
			self.randomize()
		elif 'video' in self._mode or 'distracting_cs' in self._mode:
			# apply greenscreen
			setting_kwargs = {
				'skybox_rgb': [.2, .8, .2],
				'skybox_rgb2': [.2, .8, .2],
	...
	def _load_colors(self):
		...
		self._colors = torch.load(f'/data1/wuyq/ShunQiZiRan/DMControl/src/env/data/{self._mode}.pt')
...
class FrameStack(gym.Wrapper):
	...
	def _get_obs(self):
		...
		return np.concatenate(utils.LazyFrames(list(self._frames)).frames, axis=0)
...
class VideoWrapper(gym.Wrapper):
	...
	def __init__(self, env, mode, seed):
		...
		self._index = 0
		self._video_paths = []
		if 'video' in mode or 'distracting_cs' in mode:
			self._get_video_paths()
		self._num_videos = len(self._video_paths)
		self._max_episode_steps = env._max_episode_steps

	def _get_video_paths(self):
		if 'distracting_cs' in self._mode:
			type = 'video_easy'
			video_dir = os.path.join('/data1/wuyq/ShunQiZiRan/DMControl/src/env/data', type)
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(10)]
		elif 'video_easy' in self._mode:
			video_dir = os.path.join('/data1/wuyq/ShunQiZiRan/DMControl/src/env/data', self._mode)
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(10)]
	...
	def reset(self):
		if 'video' in self._mode or 'distracting_cs' in self._mode:
			self._reset_video()
	...
	def __init__(self, env, mode):
		...
		if 'video' in mode:
			...
			if not self._video.endswith('.mp4'):
				self._video += '.mp4'
			self._video = os.path.join('/data1/wuyq/ShunQiZiRan/DMControl/src/env/data/video_easy', self._video)
