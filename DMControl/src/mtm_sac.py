...
from tools import mask_feature

import utils
from utils import PositionEmbeddingRandom, PositionalEmbedding, InverseSquareRootSchedule, AnneallingSchedule
...
class MTM(nn.Module):
    def __init__(self, predict_type, critic, augmentation, aug_prob, encoder_feature_dim, 
    ...
    patch_size, block_size):
        ...
        self.pe_layer = PositionEmbeddingRandom(12 // 2)
        point_embeddings = [nn.Embedding(1, 12) for i in range(1)]
        self.point_embeddings = nn.ModuleList(point_embeddings)

        if 'both' in predict_type  or 'baseline' in predict_type:
            self.pre_related_state_token = nn.Parameter(torch.zeros(1, 1, encoder_feature_dim)) 
            self.final_related_state_token = nn.Parameter(torch.zeros(1, 1, encoder_feature_dim))
    ...
    def spatial_Embedding(self):
        zuobiao = [(i * 12, j * 12, (i + 1) * 12 - 1, (j + 1) * 12 - 1) for j in range(7) for i in range(7)]
        boxes = torch.tensor(zuobiao, dtype=float)
        boxes = boxes + 0.5  # Shift to center of pixel
        coords = boxes.reshape(-1, 1, 4).to(self.device)
        corner_embedding = self.pe_layer.forward_with_coords(coords, (84,84))
        # print(corner_embedding.shape)
        corner_embedding[:, 0, :] += self.point_embeddings[0].weight  #(49,1,12)
        position_Embedding = torch.zeros((84,84), dtype=torch.float32, device=self.device)
        for i,box in enumerate(zuobiao):
            position_Embedding[box[0]:box[2]+1,box[1]:box[3]+1] = corner_embedding[i]
...
class MTMSacAgent(object):
    def __init__(
        ...
        ''' MTM '''
        num_heads = 1
        self.MTM = MTM(self.predict_type, 
            self.critic, augmentation, aug_prob, encoder_feature_dim, 
    ...
    def update_mtm_baseline(self, mtm_kwargs, L, step):
        pre_observation = mtm_kwargs["pre_observation"]
        final_observation = mtm_kwargs["final_observation"]
        ...
        position = self.MTM.position(T + 2).transpose(0, 1).to(self.device) # (1, T, Z) -> (T, 1, Z)
        expand_pos_emb = position.expand(T + 2, B, -1)  # (T, B, Z)

        x = observation.squeeze(-3).flatten(0, 1)
        x = self.MTM.transform(x, augment=True)
        x = self.MTM.encoder(x, add_aug=False)
        x = x.view(T, B, Z)

        final_related_state_token = self.MTM.final_related_state_token.expand(1, B, -1) #[1,128,50]
        pre_related_state_token = self.MTM.pre_related_state_token.expand(1, B, -1)

        x = torch.cat((pre_related_state_token, x), dim=0) #[16,128,50] -> [17,128,50]
        x = torch.cat((x,final_related_state_token), dim=0) #[17,128,50] -> [18,128,50]

        a_vis = action
        a_vis_size = a_vis.size(0)
        a_vis = self.MTM.action_emb(a_vis.flatten(0, 1)).view(a_vis_size, B, Z)

        x_full = torch.zeros(2 * T + 2, B, Z).to(self.device)
        x_full[0] = x[0] + expand_pos_emb[0]
        x_full[1::2] = x[1:] + expand_pos_emb[1:]
        x_full[2::2] = a_vis + expand_pos_emb[1:T+1]
        ...
        state = torch.cat([x_full[0].unsqueeze(0), x_full[1::2]], dim=0).flatten(0, 1) # (M*B, Z)

        obs = torch.cat((pre_observation, observation), dim=0)
        obs = torch.cat((obs, final_observation), dim=0)
        target_obs = obs.squeeze(-3).flatten(0, 1)
        target_obs = self.MTM.transform(target_obs, augment=True)
        with torch.no_grad():
            target_state = self.MTM.target_encoder(target_obs)
        state_loss = self.MTM.spr_loss(state, target_state)
    ...
    def update_mtm_ASAboth(self, mtm_kwargs, L, step):
        pre_observation = mtm_kwargs["pre_observation"] # [1, B, 9, 1, 100, 100]
        pre_action = mtm_kwargs["pre_action"] # [1, B, dim_A]
        observation = mtm_kwargs["observation"] # [1+self.jumps, B, 9, 1, 100, 100]
        action = mtm_kwargs["action"]   # [1+self.jumps, B, dim_A]
        reward = mtm_kwargs["reward"]   # [1+self.jumps, 1]
        final_observation = mtm_kwargs["final_observation"] # [1, B, 9, 1, 100, 100]

        T, B, C = observation.size()[:3]
        Z = self.encoder_feature_dim
    
        position = self.MTM.position(T+2).transpose(0, 1).to(self.device) # (1, T+2, Z) -> (T+2, 1, Z)
        expand_pos_emb = position.expand(T+2, B, -1)  # (T, B, Z) [18,128,50]

        x = observation.squeeze(-3).flatten(0, 1)
        x = self.MTM.transform(x, augment=True)
        position_Embedding = self.MTM.spatial_Embedding()
        x = self.MTM.encoder(x, add_aug=True, add_mask_img=self.mask_img, spatial=self.imgspatial, position_Embedding=position_Embedding)
        ...
        a_vis = torch.cat((pre_action, action), dim=0)
        a_vis_size = a_vis.size(0)
        a_vis = self.MTM.action_emb(a_vis.flatten(0, 1)).view(a_vis_size, B, Z)

        x_full = torch.zeros(3 * T + 4, B, Z).to(self.device)
        x_full[::3] = x + expand_pos_emb
        x_full[1::3] = a_vis + expand_pos_emb[:T+1,:,:]
        x_full[2::3] = a_vis + expand_pos_emb[1:,:,:]
        ...
        target_obs = self.MTM.transform(target_obs, augment=True)
        with torch.no_grad():
            target_obs = self.MTM.target_encoder(target_obs, add_aug=False, spatial=False, position_Embedding=position_Embedding)

        target_state_loss = self.MTM.spr_loss(state, target_obs)

        loss = target_state_loss
    ...
    def update_mtm(self, mtm_kwargs, L, step):
        if self.predict_type=='baseline':
            self.update_mtm_baseline(mtm_kwargs, L, step)
        if self.predict_type=='ASAboth':
            self.update_mtm_ASAboth(mtm_kwargs, L, step)
