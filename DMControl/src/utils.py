...
class ReplayBuffer(Dataset):
    ...
    def sample_spr(self):   # sample batch for auxiliary task
        idxs = np.random.randint(0,
                                 ...
                                 1 -1 if self.full else self.idx - self.jumps - 1 -1, #再减1，向前再取一个数据
                                 size=self.auxiliary_task_batch_size*2)
                                #  size=self.auxiliary_task_batch_size)
        idxs = idxs.reshape(-1, 1)
        step = np.arange(self.jumps + 1 + 1).reshape(1, -1) # this is a range
        idxs = idxs + step

        real_dones = torch.as_tensor(self.real_dones[idxs], device=self.device)   # (B, jumps+1+1, 1)
        # we add this to avoid sampling the episode boundaries
        valid_idxs = torch.where((real_dones.mean(1)==0).squeeze(-1))[0].cpu().numpy()
        idxs = idxs[valid_idxs] # (B, jumps+1+1)
        idxs = idxs[:self.auxiliary_task_batch_size] if idxs.shape[0] >= self.auxiliary_task_batch_size else idxs
        self.current_auxiliary_batch_size = idxs.shape[0]

        pre_obses = self.obses[idxs[:,0]]
        pre_obses = np.expand_dims(pre_obses, axis=1) 
        obses = self.obses[idxs[:,1:]]

        pre_obses = random_crop_2(pre_obses, self.image_size)

        obses = random_crop_2(obses, self.image_size)
        pre_obses = torch.as_tensor(pre_obses, device=self.device).float()  # (B, 1, 3*3=9, 100, 100)
        obses = torch.as_tensor(obses, device=self.device).float()   # (B, jumps+1, 3*3=9, 100, 100)
        
        next_obses = self.next_obses[idxs]

        next_obses = next_obses[:,-1]
        next_obses = np.expand_dims(next_obses, axis=1) 

        next_obses = random_crop_2(next_obses, self.image_size)
        next_obses = torch.as_tensor(next_obses, device=self.device).float()

        pre_actions = torch.as_tensor(self.actions[idxs[:,0]], device=self.device).unsqueeze(dim=1)
        ...
        spr_samples = {
            'pre_observation': pre_obses.transpose(0, 1).unsqueeze(3),
            'observation': obses.transpose(0, 1).unsqueeze(3),
            'pre_action':pre_actions.transpose(0,1),
            'action': actions.transpose(0, 1),
            'reward': rewards.transpose(0, 1),
            'final_observation':next_obses.transpose(0, 1).unsqueeze(3),
        }
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def __init__(self, num_pos_feats: int = 64, scale: Optional[float] = None) -> None:
        ...
        self.register_buffer(
            ...
            scale * torch.randn((4, num_pos_feats)),
        )
    ...
    def forward_with_coords(
        ...
        coords[:, :, 2] = coords[:, :, 0] / image_size[0]
        coords[:, :, 3] = coords[:, :, 1] / image_size[1]
